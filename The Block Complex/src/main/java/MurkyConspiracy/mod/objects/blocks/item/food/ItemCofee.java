package MurkyConspiracy.mod.objects.blocks.item.food;
...
public class ItemCofee extends ItemFood implements IHasModel, IPlantable
{
	public ItemCofee(String name, int amount, float saturation)
	{
		this(name, amount, saturation, false);
	}
	
	public ItemCofee(String name, int amount, float saturation, boolean isWolfFood)
	{
			...
			EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		{
			worldIn.setBlockState(pos.up(), BlockInit.COFFEE_PLANT.getDefaultState());
			stack.shrink(1);
